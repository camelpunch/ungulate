...
module Ungulate
  describe BlobProcessor do
    ...
    it "processes a blob into several versions and sends them to storage" do
      ...
      creator.should_receive(:create).with(blob, large).and_return('largeblob')
      creator.should_receive(:create).with(blob, medium).and_return('mediumblob')
      creator.should_receive(:create).with(blob, thumbnail).and_return('thumbnailblob')
