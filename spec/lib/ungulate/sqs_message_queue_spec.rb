...
describe Ungulate::SqsMessageQueue, :integration => true do
  def new_queue
    Ungulate::SqsMessageQueue.new(
      'some_test_queue',
  ...
  it "raises an exception if queue name not set" do
    expect { Ungulate::SqsMessageQueue.new('', {}) }.
      to raise_error(Ungulate::MissingConfiguration)
  end
end
